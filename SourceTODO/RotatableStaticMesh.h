...
class NINETOFIVE_API URotatableStaticMesh : public UStaticMeshComponent, public IHandsInterface
{
	GENERATED_BODY()
...
private:
	...
	bool cannotLock; /* Disables locking after grabbed when locked in position so it doesnt instantly release from the hand and lock again. */
	USceneComponent* grabScene; /* Scene component reference. NOTE: Spawned in when grabbed to keep track of grabbed position/rotation. */
	FTimerHandle lockingTimer; /* Locking interpolation timer. */
	float currentLockedRotation;
...
protected:
	...
	/* Check if thecurrent cumulatice angle is close enough to a locking position, if so lock the rotatable. */
