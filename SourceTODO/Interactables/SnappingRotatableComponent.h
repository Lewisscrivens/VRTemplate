...
#include "Globals.h"
#include "Interactable/SlidableStaticMesh.h"
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnCompSnapped, class AGrabbableActor*, grabbableActor);
 ...
 * NOTE: This class is good for creating floppy disc functionality or inserting coins etc. */
UCLASS(ClassGroup = (Custom), Blueprintable, BlueprintType, PerObjectConfig, EditInlineNew)
class NINETOFIVE_API USnappingSlidableComponent : public UBoxComponent
{
	GENERATED_BODY()
...
public:
	...
	FOnCompSnapped OnSnapConnect;

	/* Called when something is unsnapped. */
	UPROPERTY(BlueprintAssignable, Category = "Snapping")
	FOnCompSnapped OnSnapDisconnect;

private:

	AVRHand* handRegrab; /* The hand that just released the slidable or grabbable actor. */
	USlidableStaticMesh* slidingMesh; /* The intialised sliding static mesh. */
	AGrabbableActor* snappedGrabbable; /* The current grabbable snapped to the snapping component. */
...
protected:
	...
	void OnGrabbableGrabbed(AVRHand* hand, UPrimitiveComponent* compPressed);
